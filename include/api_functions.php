<?php
    ...
    function AddComment($msg) {
        ...
        $sql = "INSERT INTO 'Message' ('msg', 'userID') VALUES (:msg, :userID)";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':msg', $msg, SQLITE3_TEXT);
        $stmt->bindParam(':userID', $_SESSION['id'], SQLITE3_TEXT);
        if ($stmt->execute()) {
            $db->close();
            return true;
        }
    ...
    function addCombinationToDatabase() {
        ...
        $sql = "INSERT INTO 'Cocktail' ('name') VALUES (:name)";
        $stmt = $db->prepare($sql);
